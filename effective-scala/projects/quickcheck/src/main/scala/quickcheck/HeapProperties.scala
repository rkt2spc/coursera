...
trait HeapProperties(val heapInterface: HeapInterface):
  ...
  val meldingSmallHeaps: (String, Prop) =
    ...
    forAll { (x: Int, y: Int) =>
      ...
      val h1: List[Node] = insert(max, insert(max, empty))
      val h2: List[Node] = insert(min, insert(min, empty))

      val meldedHeap: List[Node] = meld(h1, h2)
  ...
  val meldingHeaps: (String, Prop) =
    def check(meldedHeap: List[Node], h1: List[Node], h2: List[Node]): Boolean =
        if isEmpty(meldedHeap) then
          isEmpty(h1) && isEmpty(h2)
        else
          val min = findMin(meldedHeap)
          if !isEmpty(h1) && min == findMin(h1) then
            check(deleteMin(meldedHeap), deleteMin(h1), h2)
          else if !isEmpty(h2) && min == findMin(h2) then
            check(deleteMin(meldedHeap), h1, deleteMin(h2))
          else
            false
