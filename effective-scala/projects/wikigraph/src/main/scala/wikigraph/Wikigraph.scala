...
final class Wikigraph(client: Wikipedia):
  ...
  def namedLinks(of: ArticleId): WikiResult[Set[String]] =
    client.linksFrom(of).flatMap((articleIds) =>
      WikiResult.traverse(articleIds.toSeq)(client.nameOfArticle)
        .map(_.toSet)
    )
  ...
  def breadthFirstSearch(start: ArticleId, target: ArticleId, maxDepth: Int): WikiResult[Option[Int]] =
    ...
    def iter(visited: Set[ArticleId], q: Queue[(Int, ArticleId)]): WikiResult[Option[Int]] = {
      if (q.isEmpty) WikiResult.successful(None)
        ...
        val ((currentDepth, currentArticleId), remainingQueue) = q.dequeue
        if (currentDepth >= maxDepth) WikiResult.successful(None)
        else {
          client.linksFrom(currentArticleId).flatMap((articleIds) => {
            if (articleIds.contains(target)) WikiResult.successful(Some(currentDepth))
            else {
              iter(
                visited + currentArticleId,
          ...
          }).fallbackTo(iter(visited + currentArticleId, remainingQueue))
        }
      }
    }
  ...
  def distanceMatrix(titles: List[String], maxDepth: Int = 50): WikiResult[Seq[(String, String, Option[Int])]] =
    WikiResult.traverse(titles)((title) => client.searchId(title).map((articleId) => (title, articleId)))
      .map((articleIDs) => {
        articleIDs.combinations(2)
          .flatMap((pair) => Seq((pair(0), pair(1)), (pair(1), pair(0))))
      })
      .flatMap((pairs) => {
        WikiResult.traverse(pairs.toSeq)((origin, target) => {
          val (originTitle, originArticleId) = origin
          val (targetTitle, targetArticleId) = target
          breadthFirstSearch(originArticleId, targetArticleId, maxDepth)
            .flatMap((distance) => WikiResult.successful(originTitle, targetTitle, distance))
        })
      })
