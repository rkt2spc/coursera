...
object TimeUsage extends TimeUsageInterface:
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) =
    ...
    val workPrefixes = Seq("t05", "t1805")
    val otherPrefixes = Seq("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")

    val (primaryNeedsColumns, r1) = columnNames.partition(name => primaryNeedsPrefixes.exists(name.startsWith))
    val (workColumns, r2) = r1.partition(name => workPrefixes.exists(name.startsWith))
    val otherColumns = r2.filter(name => otherPrefixes.exists(name.startsWith))
