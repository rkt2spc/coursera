...
class StackOverflow extends StackOverflowInterface with Serializable:
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] =
    ...
    scored
      .map(e => (firstLangInTag(e._1.tags, langs), e._2))
      .filter(_._1.isDefined)
      .map(e => (e._1.get * langSpread, e._2))
      .cache()
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] =
    ...
    val newMeansMap: Map[(Int, Int), (Int, Int)] = vectors
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] =
    ...
    val median = closestGrouped.mapValues { vs =>
      val mostCommonLang = vs.groupBy(_._1).maxBy(_._2.size)

      val langLabel: String   = langs(mostCommonLang._1 / langSpread) // most common language in the cluster
      val langPercent: Double = 100 * mostCommonLang._2.size / vs.size.toDouble // percent of the questions in the most common language
      val clusterSize: Int    = vs.size

      val sortedScores: Array[Int] = vs.map(_._2).toArray.sorted
      val medianScore: Int    = sortedScores.length % 2 match {
        // Even number of scores, take average of 2 center medians
        case 0 => (sortedScores(sortedScores.length / 2) + sortedScores(sortedScores.length / 2 - 1)) / 2
        // Odd number of scores, take the center median
        case _ => sortedScores(sortedScores.length / 2)
      }
