...
class Solution {
  ...
  private static class Problem {
    ...
    public static Problem generate(int size) {
      ...
      p.peak = rd.nextInt(size - 2) + 1;
      List<Integer> l = new ArrayList<Integer>(s);
      List<Integer> ll = l.subList(0, p.peak);
      List<Integer> rl = l.subList(p.peak, l.size());
      Collections.reverse(rl);

      p.nums = new int[size];
      System.arraycopy(ll.stream().mapToInt(Integer::intValue).toArray(), 0, p.nums, 0, ll.size());
      System.arraycopy(rl.stream().mapToInt(Integer::intValue).toArray(), 0, p.nums, p.peak, rl.size());

      p.result = rd.nextInt(p.nums.length);
      p.value = p.nums[p.result];
      return p;
    }
  }

  public static void main(String[] args) {
    Problem p = Problem.generate(Math.max(3, 10000));
    System.out.println("--------------------------------------------------------------------------------");
